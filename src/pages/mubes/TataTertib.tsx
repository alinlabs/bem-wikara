...
export function TataTertib() {
  const [tataTertibData, setTataTertibData] = useState<Bab[]>([]);
  ...
  useEffect(() => {
    fetch('/data/tatatertib.json')
      .then(res => {
        if (!res.ok) throw new Error("Gagal mengambil data");
        return res.json();
      })
      .then(data => {
        setTataTertibData(Array.isArray(data) ? data : []);
        setIsLoading(false);
      })
      .catch(err => {
        console.error("Failed to fetch Tata Tertib data:", err);
        setError("Gagal memuat data Tata Tertib. Silakan coba lagi.");
        setIsLoading(false);
      });
  }, []);
  ...
  let displayData = tataTertibData;
  if (selectedBabId !== "SEMUA") {
    displayData = tataTertibData.filter(b => b.id === selectedBabId);
  }

  // Search logic for popup
  const searchResults = tataTertibData.map(bab => {
  ...
  const handleSearchResultClick = (babId: string, pasalId: string) => {
  ...
  const searchBarUI = (
    <div className="flex items-center gap-2 md:gap-3 w-full max-w-2xl mx-auto">
      <div className="relative w-full" ref={searchRef}>
        ...
        <input 
          type="text" 
          placeholder="Cari pasal atau bab..." 
          value={searchQuery}
          onChange={(e) => {
                    ...
                    <li key={`search-${bab.id}`}>
                      ...
                      {bab.pasal.map(p => (
                        <button
                          ...
                          onClick={() => handleSearchResultClick(bab.id, p.id)}
          ...
          <div className={`${showSidebar ? 'lg:col-span-3' : 'lg:col-span-1 w-full'} space-y-3 md:space-y-8 w-full`}>
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-10 md:py-20 bg-white/50 dark:bg-slate-900/50 rounded-2xl md:rounded-3xl border border-slate-100 dark:border-slate-800">
                ...
                <p className="text-sm md:text-base text-slate-500 dark:text-slate-400"><Trans>Memuat data Tata Tertib...</Trans></p>
              </div>
            ) : error ? (
              <div className="flex flex-col items-center justify-center py-10 md:py-20 bg-white/50 dark:bg-slate-900/50 rounded-2xl md:rounded-3xl border border-slate-100 dark:border-slate-800">
                      ...
                      <ul className="text-sm space-y-1">
                        <li>
                          <button 
                            ...
                            className={`w-full flex items-center px-4 py-3 rounded-xl font-medium transition-colors text-left ${selectedBabId === "SEMUA" ? "text-primary-600 bg-primary-50 dark:bg-slate-800" : "text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}
                          >
                            <Trans>SEMUA</Trans>
                          </button>
                        </li>
                        {tataTertibData.map((bab) => (
                          <li key={bab.id}>
                            <button 
                              onClick={() => {
                                toggleBab(bab.id);
                      ...
                      <ul className="text-xs md:text-sm">
                        <li className="border-b border-slate-100 dark:border-slate-800/50">
                          <button 
                            ...
                            className={`w-full flex items-center px-3 py-3 md:px-4 md:py-3 font-medium transition-colors text-left ${selectedBabId === "SEMUA" ? "text-primary-600 bg-primary-50 dark:bg-slate-800" : "text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}
                          >
                            <Trans>SEMUA</Trans>
                          </button>
                        </li>
